/*****************************************************************
 * Deck.mjs
 * @author MacSmith22115
 * Created: Term #1 2026
 * Description: 
 *      -> Acts as a pile of playing cards cards
 *      -> Contains methods for shuffling, dealing and drawing of cards
 *      -> Ment for use with 'Card.mjs', but #cards can be an array of any datatype.
 ****************************************************************/
export default class Deck {
    #cards = []; // Array of objects representing cards in the deck

    /*****************************************************************
    * @param _cards - An Array representing all cards in the deck, passed as multiple parameters.
    *****************************************************************/
    constructor(..._cards) {
        this.#cards = _cards;
        this.shuffle();
    }

    /*****************************************************************
    * Description:
    *   -> Matches up each hand to each player, assigning each hand an owner
    * Params:
    *   -> '_hands': Array of Objects which hold arrays of cards
    *   -> '_players': Array of player IDs
    *****************************************************************/
    static assignHands(_hands, _players) {
        if (_hands.length != _players.length) {
            throw new Error(`Player-Hand Count Missmatch`);
        }
        const RESULT = {};
        _hands.forEach(_hand => {
            const INDEX = _hands.indexOf(_hand);
            const PLAYER = _players[INDEX];
            RESULT[PLAYER] = _hand;
        })
        return RESULT;
    }

    /*****************************************************************
    * Description:
    *   -> Uses the Fisher-Yates shuffling method to randomly mix up the cards in the deck
    *****************************************************************/
    shuffle() {
        let cards = this.#cards;
        for (let i = cards.length - 1; i > 0; i--) {
            const RAND_INDEX = Math.floor(Math.random() * (i + 1));
            [cards[i], cards[RAND_INDEX]] = [cards[RAND_INDEX], cards[i]];
        }
    }

    /*****************************************************************
    * Description:
    *   -> Acts as a wrapper, simpliying logic split based on deal method.  
    * Params: 
    *   -> '_count': How many hands to deal for
    *   -> '_splitRemainder': Wether Cards should ALL be dealt, or if remaining cards are splitf from the deck
    *   -> '_priorityRemoval': Array of card ids to remove FIRST if any should be removed
    *   -> '_denyRemoval': Array of card ids to NOT allow removal if any should be removed
    * Returns: Object containing an array of hands dealt, and an array of remaining cards.
    *****************************************************************/
    deal(_count, _equalSized = false, _priorityRemoval = [], _denyRemoval = []) {
        return _equalSized ? this.#dealEqual(_count, _priorityRemoval, _denyRemoval) : this.#dealAll(_count);
    }

    /*****************************************************************
    * Description:
    *   -> Deals an even number of cards to each hand, therefor producing a list of remainder cards
    * Params: 
    *   -> '_count': How many hands to deal
    *   -> '_priorityRemoval': Array of card ids to remove FIRST if any should be removed
    *   -> '_denyRemoval': Array of card ids to NOT allow for removal
    * Returns: Object containing an array of hands dealt, and an array of remaining cards.
    *****************************************************************/
    #dealEqual(_count, _priorityRemoval = [], _denyRemoval = []) {
        const TOTAL_CARDS = this.size();
        const TOTAL_PLAYERS = _count;
        const COUNT_TO_REMOVE = TOTAL_CARDS - Math.trunc(TOTAL_CARDS / TOTAL_PLAYERS) * TOTAL_PLAYERS;
        const REMOVED_CARDS = [];
        for (let i = 0; i < COUNT_TO_REMOVE; i++) {
            const PRIORITY = _priorityRemoval[i];
            if (PRIORITY != null && this.hasCard(PRIORITY)) {
                this.removeCard(PRIORITY);
                REMOVED_CARDS.push(PRIORITY);
            } else {
                let foundCard = false;
                let index = 0;
                while (!foundCard) {
                    const CARD_AT_INDEX = this.getCards()[index];
                    if (_denyRemoval.includes(CARD_AT_INDEX)) {
                        index++;
                        continue;
                    }
                    foundCard = true;
                    this.removeCard(CARD_AT_INDEX);
                    REMOVED_CARDS.push(CARD_AT_INDEX);
                }
            }
        }
        this.shuffle();
        return this.#dealAll(_count, REMOVED_CARDS);
    }

    /*****************************************************************
    * Description:
    *   -> Deals ALL cards in the deck to the hands, can produce un-equal hand sizes
    * Params: 
    *   -> '_count': How many hands to deal
    *   -> '_remainder': Optional array of remainder cards, allows for more flexibility
    * Returns: Object containing an array of hands dealt, and an array of remaining cards.
    * Throws: N/A
    *****************************************************************/
    #dealAll(_count, _remainder = []) {
        const HANDS = Array.from({ length: _count }, () => ({}));
        let playerIndex = 0;
        while (!this.isEmpty()) {
            const CARD = this.drawCard();
            HANDS[playerIndex][CARD] = CARD;
            playerIndex = (playerIndex + 1) % _count;
        }
        return { hands: HANDS, remainder: _remainder };
    }

    /*****************************************************************
    * Description:
    *   -> Draws '_count' number of cards from the top of the deck, removing them when drawn
    * Params: '_count': How many cards to draw
    * Returns: Array containing drawn cards
    *****************************************************************/
    drawCards(_count) {
        const CARDS = [];
        while (CARDS.length < _count) {
            CARDS.push(this.#cards.pop());
        }
        return CARDS;
    }

    /*****************************************************************
    * Description:
    *   -> Draws The Top card on the deck.
    * Returns: Top Card of the deck
    *****************************************************************/
    drawCard() {
        return this.drawCards(1)[0];
    }

    /*****************************************************************
    * Description:
    *   -> Computes wether this instance of 'Deck' has any held cards
    * Returns: Boolean, True if there are no cards, False if cards are present
    *****************************************************************/
    isEmpty() {
        return this.size() <= 0;
    }

    /*****************************************************************
    * Description:
    *   -> Returns an array of cards held within the deck
    * Returns: Array of Cards
    *****************************************************************/
    getCards() {
        return this.#cards;
    }

    /*****************************************************************
    * Description:
    *   -> Removes a specific card from the deck using its id
    * Params: 
    *   -> '_id': Id of card to remove
    *****************************************************************/
    removeCard(_id) {
        const INDEX = this.getCards().indexOf(_id);
        if (INDEX != -1) this.getCards().splice(INDEX, 1);
    }

    /*****************************************************************
    * Description:
    *   -> Checks if the deck has a specific card
    * Params: 
    *   -> '_card': Obj, Card to check if deck contains
    *****************************************************************/
    hasCard(_card) {
        return this.getCards().includes(_card);
    }

    /*****************************************************************
    * Description:
    *   -> Shorthand way to get how many cards are held in the deck
    *****************************************************************/
    size() {
        return this.#cards.length;
    }
}